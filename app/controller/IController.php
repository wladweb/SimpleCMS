<?php
	abstract class IController{
		...
		protected function set_template_name(){
			$this->template_name = $this->do_true_action(self::MBloginfo, 'get_template_name')['template'];
		}
		protected function set_pagination(){
			$this->pagination = $this->do_true_action(self::MBloginfo, 'get_pagination_value')['pagination'];
		}
		...
		protected function checkUser(){
			if(!empty($_COOKIE['uid']) and !empty($_COOKIE['key'])){
				$user = $this->do_true_action(self::MUsers, 'get_user_as_key', array($_COOKIE['uid'], $_COOKIE['key']));
				if($user){
					$this->setUser($user);
					$this->chek_vote();
				}else{
		...
		protected function chek_vote(){
			if(isset($_COOKIE['vv'])){
				if($_COOKIE['vv'] >= $this->vote_limit){
					$this->vote = false;
					$this->do_true_action(self::MUsers, 'set_last_vote_time',array($this->user['uid']));
					setcookie('vv','' , time()-3600,'/');
				}else{
					$this->vote = true;
				}
			}else{
				$dt = $this->do_true_action(self::MUsers, 'get_last_vote_time',array($this->user['uid']))['last_vote'];
		...
		protected function get_data(){
			$this->blog_info = $this->do_true_action(self::MBloginfo, 'get_blog_info');
			$this->category = $this->do_true_action(self::MCategory, 'get_categories_edit');
			$this->popular_posts = $this->do_true_action(self::MPosts, 'get_popular_posts');
			$this->last_comments = $this->do_true_action(self::MComments, 'get_last_comments');
