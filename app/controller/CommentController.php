<?php
	class CommentController extends IController{
		...
		public function addAction(){
			if(isset($_POST['send-message'])){
				if(!empty($_POST['message'])){
					$rows = $this->do_true_action(self::MComments, 'addComment', array($_POST['message'],$this->params['postid'],$this->params['uid'],$this->anchor_gen()));
					if(!$rows){
						$this->transporter->end_work(__CLASS__, 'ac2', '#comment-mark');
					}else{
						$this->transporter->end_work(__CLASS__, 'ac1', '#comment-mark');
					}
				}else{
		...
		public function indexAction(){
			...
			$this->comments = $this->do_true_action(self::MComments, 'get_all_comments', array((int)$this->post_start, (int)$this->coment_pagination));
		...
		public function editAction(){
			...
			if($_SERVER['REQUEST_METHOD'] === 'POST' and isset($_POST['content'])){
				...
				$r = $this->do_true_action(self::MComments, 'edit_comment', array($content,$cid));
				if($r){
		...
		public function delAction(){
			...
			if(!empty($this->params['cid'])){
				...
				$r = $this->do_true_action(self::MComments, 'delete_comment', array($cid));
				if($r){
