    ...
    public function registerAction() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!empty($login) and ! empty($pass) and ! empty($email)) {
                $r = $this->do_true_action(self::MUsers, 'addUser',
                        array($login, $pass, $email, time() - 3600 * 24, $utime));
                if ($r) {
